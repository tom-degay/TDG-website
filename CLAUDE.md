...
- **Site nav** (`.site-nav`): `position: sticky; top: 0; z-index: 50;`, translucent blurred background (`rgba(0,0,0,0.86)` + `backdrop-filter: blur(10px)`). It intentionally has **no** `border-bottom` as of this session (see below) — don't re-add one without being asked. Links: About / Approach / Case Studies (with a hover dropdown to the two case studies) / Contact.
- **Mobile nav (≤640px):** the nav links collapse behind a `.nav-toggle` hamburger button. The `.site-nav-links` list becomes an absolutely-positioned solid-black panel below the bar (`display:none` → `display:flex` when `.site-nav.is-open`), with the Case Studies dropdown shown inline/expanded. A small inline `<script>` at the end of each page's `<body>` toggles `.is-open` + `aria-expanded` and closes the menu on link click / Escape / outside click / resize above 640px. **This script and the `.nav-toggle` markup + mobile CSS are copy-pasted into all seven pages** — keep them in sync like the rest of the nav.
