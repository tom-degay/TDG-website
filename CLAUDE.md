...
- **No shared partials.** The site nav (`<nav class="site-nav">`) and its CSS are copy-pasted identically into every page's `<head>`/`<body>`. **Any change to the nav (markup, styling, links) must be manually applied to all six live pages** (see File inventory below). This is the single biggest source of drift risk in this codebase — there is no include mechanism to keep them in sync automatically.
...
- **No JS framework/dependencies anywhere else.** The only inline `<script>` in the codebase is the contact form's submit handler.
...
- **Site nav** (`.site-nav`): `position: sticky; top: 0; z-index: 50;`, translucent blurred background (`rgba(0,0,0,0.86)` + `backdrop-filter: blur(10px)`). It intentionally has **no** `border-bottom` as of this session (see below) — don't re-add one without being asked.
- **Case blocks** (`.case-block`, used on the home page preview and the case studies index): bordered rounded cards with a hover lift (`transform: scale(1.02)`, brightened border, background tint) and an arrow that translates on hover. There's an `.is-soon` variant for not-yet-published case studies (no hover lift, muted "Soon" label, `cursor: default`).
- **Sticky in-page elements:** when making something sticky within the page (not just the top nav), remember CSS `position: sticky` is bounded by the element's *immediate parent's* box — if the parent is short (e.g. wraps only a photo), the sticky element stops sticking once that parent scrolls out of view. To keep something sticky for the full page scroll, it needs to be a direct child of a full-page-height container (here, `<main>`), not nested inside a shorter wrapper. This came up directly this session (see below).

## What changed in this session (most recent → oldest)
