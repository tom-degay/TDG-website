...
- **Captioned figure:** `<figure class="figure"><img src="media/…" alt="…"><figcaption>…</figcaption></figure>`. `.figure` / `.figure img` / `.figure figcaption` styling is already in the copied CSS, and the page's lightbox script auto-wires every `.figure img`.
...
- **Compression** — tools already on Tom's Mac: `ffmpeg`/`ffprobe`, `sips`, `jpegtran`, `cwebp`. No `pngquant`/`oxipng`/`optipng`.
  - Video: `ffmpeg -i IN -an -vf "scale=<w>:-2" -c:v libx264 -profile:v high -preset slow -crf 27..28 -pix_fmt yuv420p -movflags +faststart OUT.mp4`, then a poster `ffmpeg -ss <t> -i OUT.mp4 -vframes 1 -q:v 3 poster.jpg` → `jpegtran -optimize -progressive`. Screen-capture source shrinks 90–95 % (17.8 MB → 775 KB; a 1.3 MB `.mov` → 98 KB). **`.mov` must be re-encoded** — Chrome/Firefox don't play the container reliably.
  - Large PNG spec boards: `sips -Z 2000 -s format jpeg -s formatOptions 84` then `jpegtran -optimize -progressive` (matches `feeds-add-partners-spec.jpg`, ~200 KB). The 11394-px source became a 197 KB, 2000-px JPEG.
  - `sips` has a weak PNG encoder — re-saving or downscaling an already-optimised PNG **inflates** it. The card PNGs are already compressed on export; don't try to shrink them.
- **Claude cannot save a pasted image to disk in the desktop app** — pasted images arrive only as vision input, and this client doesn't stage them to a temp path. Tom saved each asset himself (an `osascript` clipboard-to-file one-liner, or Finder) into `case-studies/media/`, then Claude wired up the markup. Filenames with spaces were renamed to kebab-case.
