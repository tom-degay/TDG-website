...
A static, no-build personal site: home page, about page, approach page, contact page, and a case studies section with three long-form case study write-ups. No framework, no bundler, no package.json — just hand-authored HTML files with inline `<style>` blocks, deployed to Vercel.
...
- **No shared partials.** The site nav (`<nav class="site-nav">`), its CSS, and its mobile-hamburger `<script>` are copy-pasted identically into every page's `<head>`/`<body>`. **Any change to the nav (markup, styling, links, toggle script) must be manually applied to all eight live pages** (see File inventory below). This is the single biggest source of drift risk in this codebase — there is no include mechanism to keep them in sync automatically. (There are now eight pages — `approach.html` and `case-studies/validation-and-correction.html` were both added after the original handover.)
...
- **No JS framework/dependencies anywhere else.** Inline `<script>` blocks in the codebase: the contact form's submit handler (`contact.html`), the section-TOC scroll-spy on the case study + approach pages, and the mobile nav hamburger toggle (all eight pages — see the mobile nav note below). Plus a GoatCounter analytics `<script>` tag near `</body>` on every page except `about.html`.
...
These eight all share the `.site-nav` sticky header (brand link + About / Approach / Case Studies [with a hover dropdown to the three case studies] / Contact) and the general dark visual language described below.
...
- **Case blocks** (`.case-block`, used on the home page preview, the case studies index, and each detail page's "Next case study" block): bordered rounded cards with a hover lift (`transform: translateY(-4px)`, brightened border, background tint) and an arrow that translates on hover. There's an `.is-soon` variant for not-yet-published case studies (no hover lift, muted "Soon" label, `cursor: default`). The lift used to be `scale(1.02)`, but a centred scale grew the card past the page's left/right alignment edges — looked like the card jumped sideways; `translateY` avoids that. The `.has-image` variant additionally zooms its image (`transform: scale(1.03)` on `.case-block-media`, inside `overflow: hidden`).
  - **Inside `.case-block-text`**, in order: `.case-block-kicker` (green `#00d8aa` IBM Plex Mono, uppercase — a short project label: **FEEDS / COLUMN MAPPING / VALIDATION / PIPELINES**, written sentence-case in the HTML and uppercased by CSS), then `.case-block-title`, `.case-block-subtitle`, and either `.case-block-keywords` (muted grey mono keyword list) or `.case-block-soon-label`. The kicker's `.case-block-kicker` rule is duplicated into all five files that render cards (`index.html`, `case-studies/index.html`, and the three detail pages) — keep in sync like the nav.
- **"Other work"** (`.other-work` / `.other-block`) — a quieter secondary list for non-Quantemplate projects, at the bottom of `index.html` **and** `case-studies/index.html` in a `<section aria-labelledby="other-work-heading">` after an `<hr class="rule">`. Currently Image Insight and Zonefully, both "coming soon" (`<div>`, not a link). Full-column-width horizontal cards — image left (`clamp(150px, 26vw, 260px)`, 16:9), text right — at `opacity: 0.7`, no hover. Its text classes are its own (`.other-block-title` / `-desc` / `-soon`) but sized to match the case-study card (`clamp(18px, 2.4vw, 22px)` title, `clamp(15px, 1.7vw, 17px)` desc, 14px mono "Coming soon"). Stacks image-on-top at ≤560px. The whole CSS block is **duplicated in both files** (and `case-studies/index.html` additionally needed an `hr.rule` rule + `#other-work-heading { margin-top: 0 }`, since that page's `.eyebrow` carries a `32px` top margin and has no `hr.rule` otherwise). Images: `other-work/media/session-review.jpg`, `other-work/media/zonefully.jpg` (that dir also has two unused PNGs, `Log in.png` / `teal nav.png`, left untracked).
...
- **The Case Studies nav dropdown is now three cards.** The third card was hand-added to all eight pages (desktop hover panel + mobile inline-expanded form). Still no include mechanism.
